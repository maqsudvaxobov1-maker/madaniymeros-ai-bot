...
def load_846_objects():
    ...
    if not os.path.isfile(path):
        logging.warning("846 structured JSON topilmadi")
        return []
    try:
        with open(path, "r", encoding="utf-8", errors="ignore") as fh:
            data = json.load(fh)
        ...
        logging.exception("846 structured JSON xatosi: %s", e)
        return []
...
def lookup_846_structured(question):
    ...
    for r in _846_OBJECTS:
        if region and r.get("viloyat") != region:
            continue
    ...
    if region and cat and not raw_tokens:
        arr = [
            ...
            if r.get("viloyat") == region and _846_category(r.get("toifa")) == cat
        ]
