...
TOPICS = {
    ...
    "arxeologiya": ["arxeolog", "arxeologiya", "qazishma", "ilmiy tadqiqot"],
...
def search_kb(question, limit=18):
    ...
    if "ekspertiza" in topics and not number:
        ...
        for doc in DOCUMENTS:
            ...
            # 26..30 — ekspertizani o'tkazish amaliy sxemasi.
            wanted = [
                s for s in sections
                if (
...
def openai_answer(question, results):
    ...
    system = (
        "Siz Madaniy Meros AI — O‘zbekiston madaniy merosi bo‘yicha huquqiy yordamchisiz.\n\n"
        "Faqat berilgan bilim bazasidagi manbalarga tayaning. Manbada yo‘q norma, band, muddat, to‘lov yoki vakolatni o‘ylab topmang.\n"
        "Savol bir nechta masalani so‘rasa, har birini alohida javoblang va barcha tegishli bandlarni birlashtiring.\n"
        "Agar savol tarixiy-madaniy ekspertiza haqida bo‘lsa, VMQ 269-sonning kontekstga berilgan BARCHA tegishli bandlarini ketma-ket tahlil qiling. Faqat band raqamini sanab o‘tirmang: har bir bandning mazmunini tushuntiring. 7-25-bandlar asosiy Nizom, 26-30-bandlar amaliy o‘tkazish sxemasi ekanini ajrating. Kim materialni kiritadi, kim ko‘rib chiqadi, seksiyalar nima qiladi, mustaqil ekspert qachon jalb qilinadi, xulosa qanday tayyorlanadi va tasdiqlanadi, majlis va bayonnoma qanday rasmiylashtiriladi, muddat va yig‘im qanday belgilanadi — barchasini kontekstdagi aniq matn asosida yoritib bering. Band raqamini yoki mazmunini almashtirmang.\n"
        "Agar savol qurilish yoki ruxsat haqida bo‘lsa, tegishli 265-son qaror va boshqa mos hujjatlarni birlashtiring.\n"
        "Javob tuzilishi: 1) Xulosa; 2) Tartib/talablar; 3) Hujjatlar yoki ruxsat; 4) Muddat/to‘lov; 5) Manbalar va aniq bandlar.\n"
        "Band raqamini faqat berilgan matndan aniq bo‘lsa ko‘rsating.\n"
        "Agar bilim bazasida javob yetarli bo‘lmasa, buni ochiq ayting.\n"
        "Javob o‘zbek tilida, aniq va amaliy bo‘lsin. Faqat “6-band, 11-band...” kabi ro‘yxat bilan cheklanib qolmang; foydalanuvchiga amaliy tartibni izchil tushuntiring. Har bir huquqiy band raqami faqat berilgan manbadagi band bilan aynan mos bo‘lsin. Markdown yulduzchalarini ishlatmang."
    )
    payload = {
        "model": OPENAI_MODEL,
        "messages": [
