...
def lookup_846_structured(question):
    ...
    # Region aliases, including Cyrillic spellings.
    regions = {
        "samarqand": "Самарқанд вилояти", "samarqand": "Самарқанд вилояти",
        "buxoro": "Бухоро вилояти", "andijon": "Андижон вилояти",
        "toshkent": "Тошкент вилояти", "fargona": "Фарғона вилояти",
        "namangan": "Наманган вилояти", "xorazm": "Хоразм вилояти",
        "qashqadaryo": "Қашқадарё вилояти", "surxondaryo": "Сурхондарё вилояти",
        "jizzax": "Жиззах вилояти", "sirdaryo": "Сирдарё вилояти", "navoiy": "Навоий вилояти",
        "qoraqalpogiston": "Қорақалпоғистон Республикаси"
    }
    region = next((v for k,v in regions.items() if k in q), None)
    cat = None
    if "arxeolog" in q or "arxeologiya" in q or "qazishma" in q:
        cat = "Археология ёдгорликлари"
    elif "arxitektura" in q: cat = "Архитектура ёдгорликлари"
    elif "monumental" in q: cat = "Монументал санъат ёдгорликлари"

    # Exact/strong object-name lookup: remove common query words and use token containment.
    stop = {"846","son","qaror","karor","qarorning","milliy","royxat","ruyxat","royxatda","royxatida","bor","bormi","obyekt","obekt","obyekti","obyektlar","obyektlari","obektlar","obektlari","pozitsiya","pozitsiyasi","manzil","manzili","qaysi","nomi","viloyati","viloyatidagi","dagi","dan","ga","bor","arxeologiya","arxeologik","qazishma","qazish","ruxsat"}
    raw_tokens=[t for t in q.split() if len(t)>2 and t not in stop]
    candidates=[]
    for r in _846_OBJECTS:
        if region and r.get("viloyat") != region: continue
        if cat and _846_category(r.get("toifa")) != cat: continue
        blob=_846_norm(f"{r.get('obyekt_toliq_nomi','')} {r.get('obyekt_manzili','')}")
        score=sum(1 for t in raw_tokens if t in blob)
        if raw_tokens and score == len(raw_tokens): candidates.append(r)
    if candidates:
        if len(candidates)==1 and raw_tokens:
            r=candidates[0]
            return ("FOUND", r)
        return ("LIST", candidates)
    # Region/category list request with no specific object token.
    if region and cat and not raw_tokens:
        arr=[r for r in _846_OBJECTS if r.get("viloyat")==region and _846_category(r.get("toifa"))==cat]
        return ("LIST", arr)
    return None
    ...
    if "846" in q or "миллий рўйхат" in q or "миллий руйхат" in q:
        ...
        if hit:
            ...
            if kind == "LIST":
                ...
                if len(arr)>25: lines.append(f"\nЖами {len(arr)} та. Кўрсатишни давом эттириш учун туман/шаҳарни аниқланг.")
                lines.append("\nМанба: VMQ 846-сон, 04.10.2019, 1-илова.")
                return "\n".join(lines)
