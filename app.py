...
if not RENDER_EXTERNAL_URL:
    hostname = (os.getenv("RENDER_EXTERNAL_HOSTNAME") or "").strip().rstrip("/")
    if hostname:
        RENDER_EXTERNAL_URL = "https://" + hostname
...
def load_kb():
    try:
        ...
        logging.error("KB JSON xatosi: %s; fayl=%s", e, KB_FILE)
        return []
    except Exception as e:
        logging.exception("KB yuklash xatosi: %s", e)
        return []
...
def load_846_source():
    ...
    for path in candidates:
        # candidates are strings because BASE_DIR is a string on Render.
        # Use os.path APIs consistently to avoid str/Path AttributeError.
...
# 846 structured national list (8,000+ records)
_846_OBJECTS = []
def load_846_objects():
    """Load the structured 846 list locally; if unavailable/corrupt, fetch the
    exact committed GitHub file once. Only app.py needs replacement.
    """
    path = os.path.join(BASE_DIR, "846_full_extracted.json")
    try:
        if os.path.isfile(path) and os.path.getsize(path) > 1000:
            with open(path, "r", encoding="utf-8-sig", errors="ignore") as fh:
                data = json.load(fh)
            objs = data.get("objects", []) if isinstance(data, dict) else []
            if isinstance(objs, list) and len(objs) >= 1000:
                logging.info("846 structured ro'yxat yuklandi: %s ta obyekt", len(objs))
                return objs
            logging.warning("846 local JSON obyektlar soni kutilganidan kam: %s",
                            len(objs) if isinstance(objs, list) else 0)
    except Exception as e:
        logging.warning("846 local JSON xatosi: %s", e)

    url = "https://raw.githubusercontent.com/maqsudvaxobov1-maker/madaniymeros-ai-bot/main/846_full_extracted.json"
    try:
        logging.info("846 structured JSON GitHub manbasidan yuklanmoqda...")
        req = urllib.request.Request(url, headers={"User-Agent": "Madaniy-Meros-AI/16"})
        with urllib.request.urlopen(req, timeout=30) as resp:
            raw = resp.read().decode("utf-8-sig", errors="strict")
        data = json.loads(raw)
        objs = data.get("objects", []) if isinstance(data, dict) else []
        if isinstance(objs, list) and len(objs) >= 1000:
            logging.info("846 GitHub fallback yuklandi: %s ta obyekt", len(objs))
            return objs
        logging.error("846 GitHub fallback noto'g'ri: obyektlar=%s",
                      len(objs) if isinstance(objs, list) else 0)
    except Exception as e:
        logging.exception("846 GitHub fallback xatosi: %s", e)
    return []

_846_OBJECTS = load_846_objects()

def _846_norm(s):
    x = norm(str(s or ""))
    return re.sub(r"[^0-9a-zа-яёғқҳўъ\s]", " ", x).strip()

def _846_category(x):
    x = _846_norm(x)
    if "arxe" in x: return "Археология ёдгорликлари"
    if "arxitektura" in x: return "Архитектура ёдгорликлари"
    if "monumental" in x and "sanat" in x: return "Монументал санъат ёдгорликлари"
    if "diqqatga" in x: return "Диққатга сазовор жойлар"
    return x

def lookup_846_structured(question):
    if not _846_OBJECTS:
        return None
    q = _846_norm(question)

    # Region aliases are normalized through _846_norm(), so both Latin and
    # Cyrillic user questions are handled consistently.
    regions = {
    ...
    cat = None
    if any(x in q for x in ("arxeolog", "qazishma", "qazish")):
        cat = "Археология ёдгорликлари"
    elif "arxitektura" in q:
        cat = "Архитектура ёдгорликлари"
    elif "monumental" in q:
        cat = "Монументал санъат ёдгорликлари"
    elif "diqqatga sazovor" in q:
        cat = "Диққатга сазовор жойлар"

    # Words that describe the request itself, not an object name.
    stop = {
        "846", "son", "qaror", "karor", "qarorning", "ilova", "ilovasi",
        "boyicha", "boyicha", "milliy", "royxat", "ruyxat", "royxatda",
        "royxatida", "royxatning", "bor", "bormi", "obyekt", "obekt",
        "obyekti", "obyektlar", "obyektlari", "obektlar", "obektlari",
        "pozitsiya", "pozitsiyasi", "manzil", "manzili", "qaysi", "nomi",
        "viloyati", "viloyatidagi", "viloyatida", "viloyat", "tuman", "tumani",
        "shahar", "shahridagi", "dagi", "dan", "ga", "uchun", "kerak",
        "arxeologiya", "arxeologik", "qazishma", "qazish", "ruxsat",
        "topildi", "ta", "obyektlari", "obektlari", "men", "siz", "menga",
        "chiqar", "korsat", "korsating", "top", "toping", "qaysilar", "qaysi"
    }
    # Remove pure numbers and request boilerplate. Region/category tokens are
    # also excluded so a regional/category list query has no false object token.
    region_tokens = set()
    if region:
        for k, v in regions.items():
            if v == region:
                region_tokens.update(k.split())
        region_tokens.update(_846_norm(region).split())
    cat_tokens = set(_846_norm(cat or "").split())
    raw_tokens = [
        t for t in q.split()
        if len(t) > 2 and not t.isdigit() and t not in stop
        and t not in region_tokens and t not in cat_tokens
    ]

    candidates = []
    for r in _846_OBJECTS:
        if region and _846_norm(r.get("viloyat")) != _846_norm(region):
            continue
        if cat and _846_category(r.get("toifa")) != cat:
            continue
        blob = _846_norm(
            f"{r.get('obyekt_toliq_nomi','')} {r.get('obyekt_manzili','')}"
        )
        score = sum(1 for t in raw_tokens if t in blob)
        if raw_tokens and score == len(raw_tokens):
            candidates.append(r)

    def sort_key(r):
        raw = str(r.get("tartib_raqami", "")).strip()
        m = re.search(r"\d+", raw)
    ...
    # Region/category list request: return ALL matching records, sorted by the
    # official list number. This prevents gaps such as 1-6, 9, 970.
    if region and cat and not raw_tokens:
        arr = [
            r for r in _846_OBJECTS
            if _846_norm(r.get("viloyat")) == _846_norm(region) and _846_category(r.get("toifa")) == cat
        ]
...
def is_specific_846_lookup(question):
    q = norm(question)
    location_words = [
        "samarqand", "самарқанд", "самарканд", "buxoro", "бухоро", "andijon", "андижон",
        "toshkent", "тошкент", "fargona", "farғona", "фарғона", "фаргона", "namangan", "наманган",
        "xorazm", "хоразм", "qashqadaryo", "қашқадарё", "surxondaryo", "сурхондарё", "jizzax", "жиззах",
        "sirdaryo", "сирдарё", "navoiy", "навоий", "qoraqalpogiston", "қорақалпоғистон"
    ]
    object_words = [
        "obyekt", "obekt", "объект", "обект", "pozitsiya", "позиция", "manzil", "манзил", "qaysi", "қайси", "кайси", "nomi", "номи",
        "ro'yxatda", "royxatda", "ruyxatda", "рўйхатда", "руйхатда", "борми", "киритилган"
    ]
...
def exact_answer(question):
    ...
    # 846 — exact structured national-list lookup.
    if "846" in q or "миллий рўйхат" in q or "миллий руйхат" in q:
        hit = lookup_846_structured(question)
        if hit:
            kind, data = hit
            if kind == "FOUND":
                r = data
                return (f"Ҳа, 846-сон қарорнинг 1-иловасида мавжуд.\n\n"
                        f"Объект: {r.get('obyekt_toliq_nomi')}\n"
                        f"Ҳудуд: {r.get('viloyat')}, {r.get('tuman_shahar')}\n"
                        f"Тоифа: {r.get('toifa')}\n"
                        f"Даври: {r.get('obyekt_davri')}\n"
                        f"Манзили: {r.get('obyekt_manzili')}\n"
                        f"Т/р: {r.get('tartib_raqami')}\n\n"
                        "Манба: VMQ 846-сон, 04.10.2019, 1-илова.")
            if kind == "LIST":
                arr=data
                lines=[f"846-сон 1-илова бўйича {len(arr)} та объект топилди."]
                for r in arr[:50]:
                    lines.append(f"{r.get('tartib_raqami')}. {r.get('obyekt_toliq_nomi')} — {r.get('tuman_shahar')}")
                if len(arr)>50: lines.append(f"\nЖами {len(arr)} та. Туман/шаҳарни айтсангиз, шу ҳудуд бўйича аниқлаб бераман.")
                lines.append("\nМанба: VMQ 846-сон, 04.10.2019, 1-илова.")
                return "\n".join(lines)

    # 846 — generic question: give only source-backed identity.
    if "846" in q and not is_specific_846_lookup(question):
        return (
            "846-son qaror — O‘zbekiston Respublikasi Vazirlar Mahkamasining "
            "2019-yil 4-oktabrdagi qarori bo‘lib, moddiy madaniy merosning "
            "ko‘chmas mulk obyektlari milliy ro‘yxatini tasdiqlashga qaratilgan.\n\n"
            "Eslatma: ushbu ishchi bazada 846-sonning to‘liq milliy ro‘yxati "
            "alohida manba sifatida ulanmagan. Shu sababli aniq obyekt yoki "
            "pozitsiya bo‘yicha xulosa bermayman.\n\n"
            "Manba: VMQ 846-son, 04.10.2019."
        )
    ...
    if "846" in q or "milliy royxat" in q or "milliy ro‘yxat" in q:
        if _846_DOC is None and is_specific_846_lookup(question):
            return (
...
- Faqat ISHONCHLI KONTEKSTdagi matn bilan asosla.
- Kontekstda yo‘q raqam, sana, band, to‘lov, muddat, vakolat yoki ruxsat turini yaratma.
- Qonun, qaror, nizom, xulosa, kelishuv va ruxsatnomani bir xil hujjat deb hisoblama.
- 269-II Qonun, 269-son qaror/Nizom, 295-son, 119-son, 265-son, 649-son va 846-sonni alohida ajrat.
- Agar ikki hujjat vakolat yoki tartib bo‘yicha farq qilsa, qaysi biri amalda ekanini taxmin qilma; manbada aniq asos bo‘lmasa, bu cheklovni ochiq ayt.
- “Amaliyotda”, “ehtimol”, “mumkin” kabi iboralar bilan normativ xulosani almashtirma.
- Arxeologik qazishma bo‘yicha maxsus ruxsatnoma, ochiq varaq va Kengash xulosasi tushunchalarini alohida ko‘rsat.
- CCTV yoki boshqa texnik montajni manbada bevosita norma bo‘lmasa avtomatik ravishda qurilish deb tasniflama.
- 846-son ro‘yxatdagi obyekt faqat to‘liq ro‘yxat manbasida aynan tasdiqlangan bo‘lsa “bor” deb aytiladi. Topilmagan obyektni “yo‘q” yoki “madaniy meros emas” deb xulosa qilma.
- “Manba 1”, “Manba 2” kabi ichki texnik belgilarni foydalanuvchiga chiqarmasdan, hujjatning nomi, sanasi, raqami va bandini yoz.
- Manba yetarli bo‘lmasa: “Taqdim etilgan bilim bazasida bu savolga yetarli aniq ma’lumot topilmadi” deb ayt.
"""
    context = context + "\n\n" + strict_suffix
